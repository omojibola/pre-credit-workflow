...
export function RFQPanel({
  ...
  useEffect(() => {
    if (!rfqState?.sentAt || !['PENDING', 'QUOTED'].includes(rfqState.status))
      return;
    const tick = () =>
      setCountdown(
        Math.max(
          0,
          Math.ceil((TTL_MS - (Date.now() - rfqState.sentAt)) / 1000),
        ),
      );
    tick();
  ...
  const bestBidVal = liveQ.length
    ? Math.max(...liveQ.map((q) => q.bid!))
    : null;
  const bestAskVal = liveQ.length
    ? Math.min(...liveQ.map((q) => q.ask!))
    : null;
  const bestBidId =
    bestBidVal != null
      ? liveQ.find((q) => q.bid === bestBidVal)?.dealerId
      : null;
  const bestAskId =
    bestAskVal != null
      ? liveQ.find((q) => q.ask === bestAskVal)?.dealerId
      : null;
    ...
    >
        ...
        {rfqState && (
          ...
          >
            {status === 'PENDING'
              ? `AWAITING (${pendingCt})`
              : status === 'QUOTED'
                ? `LIVE (${liveQ.length}/${rfqState.dealerIds.length})`
                : (status ?? '')}
          </span>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px' }}>
        ...
        <div style={{ marginBottom: 10 }}>
          <div style={{ color: '#e2b96b', fontSize: 13, fontWeight: 600 }}>
            {instrument.issuer}
          </div>
          <div
            style={{ color: '#252b38', fontSize: 10, fontFamily: 'monospace' }}
          >
            {instrument.id} · {instrument.rating} · {instrument.sector}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          <input
            ...
            step={1e6}
            value={notional}
            onChange={(e) => !isSent && setNotional(+e.target.value)}
            disabled={isSent}
        ...
        >
          {(
            [
              ...
              [
                'BID / ASK',
                price ? `${f2(price.bid)} / ${f2(price.ask)}` : '—',
              ],
              [
                '½ SPREAD',
                price ? `${f2((price.ask - price.bid) / 2)}bps` : '—',
              ],
        ...
        {isSent && quotes.length > 0 && (
          <div style={{ marginBottom: 10 }}>
            ...
            <div
              style={{
                display: 'grid',
                gridTemplateColumns: '72px 64px 64px 64px 72px 52px',
                padding: '3px 10px',
                background: '#0d0d0f',
          ...
          >
            ...
            {(
              [
                ...
                [
                  'COVER',
                  rfqState.coverPrice ? `${f2(rfqState.coverPrice)}bps` : '—',
                ],
              ] as [string, string][]
            ).map(([k, v]) => (
              <div
        ...
        {status === 'EXPIRED' && (
          ...
          >
            ...
            <button
              onClick={onNewRFQ}
              style={{ ...mkBtn('#f97316'), padding: '5px 18px' }}
            >
              REQUEST AGAIN
            </button>
          </div>
        ...
        {isIdle && (
          <button
            onClick={() =>
              !wouldBreach &&
              onSendRFQ(instrument, side, notional, mid, dealerCount)
            }
            style={{
              width: '100%',
              padding: '10px 0',
              background: wouldBreach
                ? '#120a0a'
                : 'linear-gradient(90deg,#0e1e2e,#122334)',
              border: `1px solid ${wouldBreach ? '#3a1010' : '#1e3040'}`,
              color: wouldBreach ? '#f87171' : '#e2b96b',
              fontWeight: 700,
              fontSize: 12,
              letterSpacing: 1.5,
              cursor: wouldBreach ? 'not-allowed' : 'pointer',
              borderRadius: 3,
              fontFamily: 'inherit',
            }}
          >
            {wouldBreach
              ...
              : `SEND RFQ TO ${dealerCount} DEALERS →`}
          </button>
        )}
        {status === 'PENDING' && (
          <div
