...
export const BlotterRow = memo(
    ...
    return (
      ...
      >
        <div>
          ...
          <div
            style={{ color: '#242a38', fontSize: 9, fontFamily: 'monospace' }}
          >
            {id}
          </div>
        </div>
        <div>
          <span
            style={{
              ...
              padding: '2px 4px',
              borderRadius: 2,
              background: SECTOR_BG[sector] ?? '#1a1a1a',
              color: SECTOR_FG[sector] ?? '#666',
            }}
          >
            {sector}
          </span>
        ...
        <div style={{ textAlign: 'right', paddingRight: 4 }}>
          <FlashCell
            value={price?.bid ?? null}
            direction={price?.bidFlash ?? 0}
          />
        </div>
        <div style={{ textAlign: 'right', paddingRight: 4 }}>
          <FlashCell
            value={price?.mid ?? null}
            direction={price?.midFlash ?? 0}
          />
        </div>
        <div style={{ textAlign: 'right', paddingRight: 4 }}>
          <FlashCell
            value={price?.ask ?? null}
            direction={price?.askFlash ?? 0}
          />
        </div>
        <div
          style={{
  ...
  (p, n) => {
    ...
    return (
      p.price.bid === n.price.bid &&
      p.price.ask === n.price.ask &&
      p.price.mid === n.price.mid
    );
  },
);
