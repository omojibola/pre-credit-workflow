...
export function Blotter({ instruments, prices, sessionHiLo, selected, sort, onSelect, onSort }: Props) {
  ...
  const visible = useMemo(() => instruments.slice(start, end + 1), [instruments, start, end]);

  return (
    <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, overflow:"hidden", borderRight:"1px solid #0f0f12" }}>
      {/* Column headers */}
      <div style={{ background:"#08090b", borderBottom:"1px solid #111116", height:26, flexShrink:0, display:"flex", alignItems:"center", paddingLeft:14 }}>
        {COLUMNS.map((c) => {
          ...
          return (
            <div
              ...
              style={{
                ...
                padding: "0 5px", userSelect:"none",
                cursor:  sortable ? "pointer" : "default",
                display: "flex", alignItems:"center",
                justifyContent: c.align === "right" ? "flex-end" : "flex-start",
                gap: 3,
                color:   sort.key === c.key ? "#e2b96b" : "#1e2530",
                fontSize:9, fontWeight:700, letterSpacing:1, transition:"color .2s",
              }}
            >
              {c.label}
              {sortable && (
                <span style={{ fontSize:8, opacity: sort.key === c.key ? 1 : 0.3 }}>
      ...
      {/* Footer */}
      <div style={{ background:"#08090b", borderTop:"1px solid #0f0f12", height:22, display:"flex", alignItems:"center", paddingRight:12, fontSize:9, fontFamily:"monospace", color:"#141820", justifyContent:"flex-end" }}>
        {end - start + 1}/{instruments.length} RENDERED · VIRTUAL SCROLL · RAF BATCH
      </div>
    </div>
  );
}
